...
import { LANGUAGE_OPTIONS, CHARACTERISTIC_OPTIONS } from '../constants';
...
const AgentManager: React.FC<AgentManagerProps> = ({ agent, setAgent, navigate }) => {
  ...
  const [isNewlyCreated, setIsNewlyCreated] = useState(false);  // FIXED: Track if agent just created
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        ...
        // Backend returns { success: true, agents: [...] } — extract the array
        const agentArray: AgentConfig[] = response.agents || response || [];
        // Normalize: map _id → id for consistency
        const normalizedAgents = agentArray.map((a: any) => ({
          ...a,
          id: a.id || a._id,
        }));
        setAgentList(normalizedAgents);

        // CRITICAL FIX: The agent loaded from localStorage may belong to a different
        // user (stale data from a previous session or account). After fetching the real
        // backend list, if the current localAgent ID is not in that list, switch to the
        // first real agent that actually belongs to this user.
        if (normalizedAgents.length > 0) {
          const currentIdExistsInBackend = normalizedAgents.some(
            (a: AgentConfig) => a.id === agent.id
          );
          if (!currentIdExistsInBackend) {
            console.warn(
              `⚠️ [fetchAgents] LocalStorage agent ID "${agent.id}" not found in backend list. ` +
              `Switching to first real agent: "${normalizedAgents[0].id}"`
            );
            const firstAgent = normalizedAgents[0];
            setLocalAgent(firstAgent);
            setAgent(firstAgent);
          }
        }
  ...
  const handleSave = async () => {
    try {
      ...
      // CRITICAL: Log what agent we're about to save
      console.log('💾 [handleSave] STARTING SAVE');
      console.log('   localAgent.id:', localAgent.id);
      console.log('   localAgent.name:', localAgent.name);
      console.log('   Is New?', !localAgent.id || localAgent.id === 'new');

      // Check if this is a NEW agent or EXISTING agent
      if (!localAgent.id || localAgent.id === 'new') {
        // NEW AGENT: Call createAgent
        console.log('📝 [handleSave] Creating new agent...');
        const createResponse = await createAgent(localAgent);
        savedAgent = createResponse.agent || createResponse;

        // Add to list
        setAgentList(prev => [...prev, savedAgent]);
      } else {
        // EXISTING AGENT: Call updateAgent
        console.log('✏️ [handleSave] Updating existing agent with ID:', localAgent.id);
        console.log('   Agent to update:', { id: localAgent.id, name: localAgent.name });
        const updateResponse = await updateAgent(localAgent.id, localAgent);
        savedAgent = updateResponse.agent || updateResponse;
      ...
      console.log('✅ [handleSave] Save successful:', savedAgent.id);
      setAgent(savedAgent);
      setIsSaved(true);
      setIsNewlyCreated(false);  // FIXED: Clear flag - agent can now be navigated away from

      // Visual feedback
      setTimeout(() => setIsSaved(false), 3000);

    } catch (error) {
      console.error('❌ [handleSave] Failed to save agent:', error);
  ...
  const handleSelectAgent = async (selectedAgent: AgentConfig) => {
    // FIXED (2026-02-27): Prevent selecting different agent if one was just created
    if (isNewlyCreated) {
      alert('⚠️ Please save the newly created agent first before switching to another agent.');
      console.warn('[handleSelectAgent] Blocked: Newly created agent must be saved first');
      return;
    }
    ...
    if (!isSaved) {
      if (window.confirm("You have unsaved changes that will be lost. Do you want to continue?")) {
        ...
        try {
          const freshResponse = await getAgentById(selectedAgent.id);
          const freshAgent = freshResponse.agent || freshResponse;
  ...
  if (agentList.length >= planRules.maxAgents) {
    ...
    try {
      ...
      console.log('📝 [handleAddAgent] Creating agent with data:', newAgentData);
      const createdAgent = await createAgent(newAgentData);
      console.log('✅ [handleAddAgent] Agent created:', { id: createdAgent.id, name: createdAgent.name });

      // Update local list
      setAgentList(prev => [...prev, createdAgent]);

      // Select the newly created agent - CRITICAL: Keep this exact reference
      console.log('🔧 [handleAddAgent] Setting localAgent to newly created agent:', createdAgent.id);
      setLocalAgent(createdAgent);
      setAgent(createdAgent);
      setIsSaved(false);  // Mark as unsaved (needs initial save)
      setIsNewlyCreated(true);  // FIXED: Mark as newly created - prevent navigation until saved

      // Verify the state was set correctly
      setTimeout(() => {
        console.log('🔍 [handleAddAgent] Verification: localAgent should be', createdAgent.id);
        console.log('🔍 [handleAddAgent] isNewlyCreated flag set to:', true);
      }, 100);
  ...
  const removeFile = async (index: number) => {
    ...
    try {
      if (docToRemove?.id && localAgent.id) {
        await deleteKnowledgeDoc(localAgent.id, docToRemove.id);
      }
  ...
  return (
    ...
    <div className="flex h-[calc(100vh-100px)] bg-slate-50 border border-slate-200 rounded-xl overflow-hidden shadow-sm">
               ...
               return (
                  ...
                  >
                     <div className="relative">
                       <div className={`w-10 h-10 rounded-full bg-blue-600 border border-white shadow-sm flex items-center justify-center text-white text-base font-bold select-none ${isFrozen ? 'grayscale' : ''}`}>
                         {(agt.name || 'A').charAt(0).toUpperCase()}
                       </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
                    ...
                    <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                       ...
                       <div className="col-span-1">
                          <label className="block text-sm font-medium text-slate-700 mb-2 flex items-center">
                             Agent Avatar <Info className="w-3 h-3 ml-1 text-slate-400"/>
                          </label>
                          <div className="flex items-center space-x-4">
                             <div className="w-16 h-16 rounded-full bg-blue-600 flex items-center justify-center text-white text-2xl font-bold shadow-sm select-none">
                               {(localAgent.name || 'A').charAt(0).toUpperCase()}
                             </div>
                             <p className="text-sm text-slate-500">Avatar is auto-generated from agent name.</p>
                          </div>
                       </div>
                          ...
                          <div className="relative">
                             ...
                             >
                                {isLoadingVoices ? (
                                  ...
                                  voicesList.map(v => <option key={v.id} value={v.id}>{v.displayName} ({v.gender === 'FEMALE' ? 'Female' : 'Male'}) — {v.characteristic}</option>)
                                ) : (
                       ...
                       <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                          ...
                          <div>
                             ...
                             <div className="relative">
                                <input
                                  type="number"
                                  value={Math.round((localAgent.silenceDetectionMs || 30000) / 1000)}
                                  onChange={(e) => handleChange('silenceDetectionMs', parseInt(e.target.value) * 1000)}
                                  className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none pr-10 transition-shadow"
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">sec</span>
                             </div>
        ...
        {isTestAgentOpen && (
          <TestAgentModal
            agentId={localAgent.id || localAgent._id || ''}
            agentName={localAgent.name || localAgent.agentName || 'Test Agent'}
