...
const LeadManager: React.FC = () => {
  ...
  const [csvAgentId, setCsvAgentId] = useState<string>("");
  const [showCsvAgentPicker, setShowCsvAgentPicker] = useState(false);
  ...
  useEffect(() => {
    getAgents()
      .then((data: any) => setAgents(Array.isArray(data) ? data : data?.agents || []))
      .catch(() => setAgents([]));
  }, []);
  ...
  const loadLeads = async () => {
    try {
      ...
      setLeads(Array.isArray(data.contacts) ? data.contacts : []);
    } catch (err) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    // Show agent picker before importing
    setPendingCsvFile(file);
    setCsvAgentId("");
    setShowCsvAgentPicker(true);
  };

  const handleCsvImportConfirm = async () => {
    if (!pendingCsvFile) return;
    setShowCsvAgentPicker(false);

    try {
      const formData = new FormData();
      formData.append("file", pendingCsvFile);
      if (csvAgentId) formData.append("agentId", csvAgentId);
  ...
  const handleAddManualLead = async (e: React.FormEvent) => {
    ...
    if (newLead.agentId) payload.agentId = newLead.agentId;

    try {
      if (editingLeadId) {
        await apiFetch(`/api/contacts/${editingLeadId}`, {
          method: "PUT",
      ...
      await loadLeads(); // Always re-fetch from server so list reflects DB truth
      resetForm();
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredLeads.map((lead, idx) => {
                  const isNearBottom = idx >= filteredLeads.length - 2;
                  return (
                    ...
                    <td className="p-4 text-right relative">
                      ...
                      {openMenuId === lead.id && (
                        <>
                          ...
                          />
                          <div className={`absolute right-0 bg-white border rounded-lg shadow-lg z-20 w-32 ${isNearBottom ? "bottom-full mb-1" : "mt-2"}`}>
                            <button
      ...
      {/* CSV AGENT PICKER MODAL */}
      {showCsvAgentPicker && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-sm">
            <h2 className="text-lg font-bold mb-4">Import CSV</h2>
            <p className="text-sm text-slate-500 mb-4">
              Optionally assign an AI agent to all imported contacts.
            </p>
            {agents.length > 0 ? (
              <select
                className="w-full border rounded-lg p-2 bg-white text-sm mb-4"
                value={csvAgentId}
                onChange={e => setCsvAgentId(e.target.value)}
              >
            ...
            <div className="flex gap-2">
              <button
                onClick={() => { setShowCsvAgentPicker(false); setPendingCsvFile(null); if (fileInputRef.current) fileInputRef.current.value = ""; }}
                className="flex-1 border px-4 py-2 rounded-lg hover:bg-slate-50 text-sm"
              >
                Cancel
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAddManualLead} className="space-y-4">
              ...
              <input
                placeholder="Industry (e.g. Healthcare, Finance, Retail)"
                className="w-full border rounded-lg p-2"
