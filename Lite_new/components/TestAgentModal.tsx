...
export const TestAgentModal: React.FC<TestAgentModalProps> = ({ agentId, agentName, onClose }) => {
  ...
  const playAudio = (base64Audio: string, sampleRate: number) => {
    try {
      ...
      // Queue audio with its actual sample rate for correct playback speed
      audioQueueRef.current.push({ data: float32Array, sampleRate });
  ...
  const playQueuedAudio = () => {
    ...
    try {
      ...
      // Use actual sample rate from Gemini (24000Hz) — not hardcoded 48000
      // Mismatch was causing 2x playback speed
      const audioBuffer = audioContextRef.current.createBuffer(1, chunk.data.length, chunk.sampleRate);
