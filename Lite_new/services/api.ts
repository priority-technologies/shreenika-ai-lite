...
export const updateAgent = (id: string, payload: any) => {
  // Strip MongoDB system fields that cause backend validation to fail
  const { _id, __v, createdAt, updatedAt, ...cleanPayload } = payload;
  return apiFetch(`/api/voice/agents/${id}`, {
    method: "PUT",
...
export const uploadKnowledgeFile = (file: File, agentId: string, title?: string) => {
  ...
  return apiFetch(`/api/voice/agents/${agentId}/knowledge-base/upload`, {
    method: "POST",
    body: formData,
  }, "core");
};
...
export const listKnowledgeDocs = (agentId: string) =>
  apiFetch(`/api/voice/agents/${agentId}/knowledge-base`, undefined, "core");

export const deleteKnowledgeDoc = (agentId: string, docId: string) =>
  apiFetch(`/api/voice/agents/${agentId}/knowledge-base/${docId}`, { method: "DELETE" }, "core");
...
// ── MISSING FIX #4: Fetch recording paths from backend ──
export const getCallRecording = (callId: string) =>
  apiFetch(`/calls/${callId}/recording`, undefined, "core");
...
export const getCampaigns = () =>
  apiFetch("/campaigns", undefined, "core");

export const getCampaignDetails = (campaignId: string) =>
  apiFetch(`/campaigns/${campaignId}`, undefined, "core");

export const getCampaignStatus = (campaignId: string) =>
  apiFetch(`/campaigns/${campaignId}/status`, undefined, "core");

export const createCampaign = (payload: {
  agentId: string;
  campaignName: string;
  configurationSheet?: string;
  ...
  apiFetch("/campaigns", {
    ...
    body: JSON.stringify({
      ...
      description: payload.configurationSheet || "",
      leads: payload.leads,
    }),
  }, "core");

export const startCampaign = (campaignId: string) =>
  apiFetch(`/campaigns/${campaignId}/start`, {
    method: "POST",
  }, "core");

export const pauseCampaign = (campaignId: string) =>
  apiFetch(`/campaigns/${campaignId}/pause`, {
    method: "POST",
  }, "core");

export const stopCampaign = (campaignId: string) =>
  apiFetch(`/campaigns/${campaignId}/stop`, {
    method: "POST",
  }, "core");

export const resumeCampaign = (campaignId: string) =>
  apiFetch(`/campaigns/${campaignId}/resume`, {
    method: "POST",
  }, "core");
...
export const createRechargeIntent = (minutes: number) =>
  apiFetch("/api/billing/recharge", {
    method: "POST",
    body: JSON.stringify({ minutes }),
  }, "core");

export const createRechargeCheckout = (minutes: number) =>
  apiFetch("/api/billing/recharge-checkout", {
    method: "POST",
    body: JSON.stringify({ minutes }),
  }, "core");

export const createAddonCheckout = (type: string) =>
  apiFetch("/api/billing/addon-checkout", {
    method: "POST",
...
// ==============================
export const generateApiKey = (_name?: string) =>
  apiFetch("/api/auth/api-key", {
    method: "POST",
  }, "core");

export const listApiKeys = () =>
  apiFetch("/api/auth/api-key", undefined, "core");

export const revokeApiKey = (_id: string) =>
  apiFetch("/api/auth/api-key", {
    method: "DELETE",
  }, "core");
